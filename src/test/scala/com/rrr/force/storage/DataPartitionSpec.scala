...
class DataPartitionSpec extends AnyFunSuite {
  test("DataPartition.load should read NDJSON file and parse all event types") {
    ...
    // 3. Write NDJSON lines to the file
    val ndjson = Seq(pushLine, watchLine, createLine, issuesLine).mkString("\n")
    Files.write(file, ndjson.getBytes(StandardCharsets.UTF_8), StandardOpenOption.CREATE)

    // 4. Load and verify
    val dp = DataPartition.load(tmpDir.toString, 0)
    assert(dp.id == 0, "Partition ID should be 0")
    assert(dp.events.size == 4, "Should parse four events")
    ...
    dp.events(0) match {
      ...
      case other =>
        fail(s"Expected PushEvent, got: $other")
    }

    dp.events(1) match {
      case we: WatchEvent =>
        assert(we.id        == "2")
        assert(we.eventType == "WatchEvent")
        assert(we.actor.login == "user2")
        assert(we.action    == "started")
        assert(we.createdAt == Instant.parse(nowStr))
      case other =>
        fail(s"Expected WatchEvent, got: $other")
    }

    dp.events(2) match {
      case ce: CreateEvent =>
        assert(ce.id         == "3")
        assert(ce.eventType  == "CreateEvent")
